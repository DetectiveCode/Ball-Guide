/* UPDATED BY FARBOD DANESHYAN for Box2D 2.3.2 for SFML 2.4.1 */

// Debug draw for Box2D 2.2.1 - 2.3.0 for SFML 2.0 - DebugDraw.cpp
...
{
	sf::CircleShape point(size * sfdd::SCALE);
	point.setOrigin(size * sfdd::SCALE, size * sfdd::SCALE);
	point.setFillColor(DebugDraw::GLColorToSFML(color));
	point.setPosition(DebugDraw::B2VecToSFVec(p));
...
{
	b2Vec2 p1 = b2Vec2(aabb->lowerBound.x * sfdd::SCALE, aabb->lowerBound.y * sfdd::SCALE);
	b2Vec2 p2 = b2Vec2(aabb->upperBound.x * sfdd::SCALE, aabb->lowerBound.y * sfdd::SCALE);
	b2Vec2 p3 = b2Vec2(aabb->upperBound.x * sfdd::SCALE, aabb->upperBound.y * sfdd::SCALE);
	b2Vec2 p4 = b2Vec2(aabb->lowerBound.x * sfdd::SCALE, aabb->upperBound.y * sfdd::SCALE);

	sf::Vertex vertices[] = 
	{
		sf::Vertex(DebugDraw::B2VecToSFVec(p1), DebugDraw::GLColorToSFML(c)),
		sf::Vertex(DebugDraw::B2VecToSFVec(p2), DebugDraw::GLColorToSFML(c)),

		sf::Vertex(DebugDraw::B2VecToSFVec(p2), DebugDraw::GLColorToSFML(c)),
		sf::Vertex(DebugDraw::B2VecToSFVec(p3), DebugDraw::GLColorToSFML(c)),

		sf::Vertex(DebugDraw::B2VecToSFVec(p3), DebugDraw::GLColorToSFML(c)),
		sf::Vertex(DebugDraw::B2VecToSFVec(p4), DebugDraw::GLColorToSFML(c)),

		sf::Vertex(DebugDraw::B2VecToSFVec(p4), DebugDraw::GLColorToSFML(c)),
		sf::Vertex(DebugDraw::B2VecToSFVec(p1), DebugDraw::GLColorToSFML(c))
	};

	m_window->draw(vertices, 8, sf::Lines);
}